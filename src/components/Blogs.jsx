import { Grid, Paper, Box, Stack, Typography, Divider, Button, useMediaQuery } from "@mui/material";
...
const BlogPost = ({ id, title, excerpt, author, image, date, url }) => {
  const classes = useStyles();

  return (
    <Grid item xs={12} sm={6} md={4}>
      <Paper className={classes.paper} sx={{ boxShadow: '0px 5px 10px rgba(0, 0, 0, 0.09), 0px 2px 5px rgba(0, 0, 0, 0.1)' }} >
        <img src={image} alt={title} className={classes.image} />
        <Typography variant="h5" sx={{ color: '#3D2521' }}>{title}</Typography>
        <Typography variant="body2" color="textSecondary" sx={{ fontFamily: 'Poppins', m: { xs: " 4vw 0vw", sm: "1vw 0vw" } }}>
          By {author} | {date}
        </Typography>
        <Typography variant="body2" sx={{ fontFamily: 'Poppins', m: { xs: " 4vw 0vw", sm: "1vw 0vw" } }}>{excerpt}</Typography>
        <Button
          variant="contained"
          color="primary"
          component={Link}
